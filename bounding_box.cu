...
//这算法的意思是可以给定数个点，可以生成最大和最小对角线组成一个box。生成的box的左下角和右上角并非是原来的坐标，就是最小的x坐标，最小的y坐标，最大的x坐标，最大的y坐标生成的。这个搞法其实我们在后来的坐标运算中能用得到。但爷爷还是没搞清楚这里面结构体的关系，很复杂。有点难。
struct point2d
{
  int x, y;
...
int main(void)
{
  ...
  thrust::uniform_real_distribution<float> u01(1, 3);
